<template>
  <Head title="Form Visitor" />
  <MainLayout title="Form Visitor">
      <div>
          <el-form ref="formRef" :model="form" :rules="rules" label-width="200px" label-position="top" require-asterisk-position="right" autocomplete="off">
              
              <!-- Email field -->
              <el-form-item :error="getFormError('email')" prop="email" label="Email" :required="true">
                  <el-input v-model="form.email" autocomplete="one-time-code" placeholder="example@gmail.com" autocorrect="off" spellcheck="false" class="!w-full" />
              </el-form-item>
  ...
  <el-form-item :error="getFormError('ms_departeman_id')" prop="ms_departeman_id" label="Departemen / Department" :required="true">
    <el-select v-model="form.ms_departeman_id" placeholder="Select" class="!w-full">
      ...
      />
              ...
              <el-form-item :error="getFormError('jpemohon')" prop="jpemohon" label="Jumlah Visitor" :required="true">
                  <el-input v-model="form.jpemohon" autocomplete="one-time-code" autocorrect="off" type="number" spellcheck="false" class="!w-full" />
              </el-form-item>
  ...
  <div v-for="(identity, index) in form.identities" :key="index" class="identity-container space-y-4">
    ...
    <el-form-item :error="getFormError(`identities.${index}.media_identitas`)" prop="identities.media_identitas" label="Masukkan (KTP / Passport) dan (Surat Pengajuan / Submission letter)" :required="true">
      <el-upload
        drag
        ...
        :auto-upload="false"
        :limit="3"
        accept="image/jpeg,image/png,pdf/application"
      >
    ...
    <el-button type="danger" @click="removeIdentity(index)" class="remove-identity-button">Hapus Identitas</el-button>
  </div>
  <!-- <el-button type="button" @click="addIdentity" class="add-identity-button">Tambah Identitas</el-button> -->
</el-card>


    <!-- Field untuk nama, email, dll. -->
    <el-checkbox v-model="form.status" true-value="0" false-value="1">Simpan sebagai Draft</el-checkbox>
...
// const selectedKompartemenId = ref(null);
// const filteredDepartemens = ref([]);
const kompartemens = page.props.kompartemenOptions;
const departemens = page.props.departemenOptions;



// Watch jpemohon and adjust identities array
watch(() => form.jpemohon, (newJpemohon) => {
  if (newJpemohon && newJpemohon > 0) {
    const identities = [];
    for (let i = 0; i < newJpemohon; i++) {
      identities.push({ name: '', phone: '', media_identitas: [] });
    }
...
.identity-container {
  margin-bottom: 20px; /* Space between each identity block */
}

.remove-identity-button {
  margin-top: 5px; /* Add space above the 'Hapus Identitas' button */
  margin-bottom: 5px; /* Add space below the 'Hapus Identitas' button */
}

.add-identity-button {
  margin-top: 5px; /* Add space above the 'Tambah Identitas' button */
}
...
.submit-button-container {
  ...
  margin-bottom: 0; /* Ensure it's at the bottom */
}
